      ...
      Command.Let_syntax.(
        ...
        fun () ->
          try
            ...
            let ast =
              if Filename.check_suffix filename ".mlb" then
                I.parse Infra.Mlb text
            ...
            if run then
              ...
              Assemble.build directory I.runtime_object_file filename instrs
              |> ignore
          with Stuck _ as e -> Printf.eprintf "Error: %s\n" (Exn.to_string e))
    ...
    Command.basic ~summary:"Interpret the given file"
      Command.Let_syntax.(
        let%map_open filename =
          anon (maybe ("filename" %: Command.Param.string))
        and expression =
        ...
        fun () ->
          try
            ...
            | _ ->
                Printf.eprintf
                  "Error: must specify either an expression to evaluate or a \
                   file\n"
